...
async function createServer(routesDir: string, config: ServerConfig = defaultConfig) {
  ...
  const server = isDev 
    ...
    : https.createServer({/* HTTPs options  */}, requestListener); // TODO: add HTTPS options

  server.listen(port, () => {
    console.log(`🚀 Server started on ${isDev ? 'http' : 'https'}://localhost:${port} ${isDev ? "(dev)" : ""}`);
  });
